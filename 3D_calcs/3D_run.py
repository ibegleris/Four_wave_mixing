# -*- coding: utf-8 -*-
"""
This code, given the inverse group velocity of the LP01 and LP11 modes finds the optimum
...
def calc_overlaps(B,w_vec):
    """
    Calculates all possible overlap combinations found in Agrawals book.
    Inputs::
        B(list of strings) :: In the order of pump1, pump2, signal, idler. Beta holds what wave is is what mode.
        w_vec[array of floats] :: The width of the mode
    Local::
        B_func[list of functions]:: A list of the hermit gaussian functions in the order of B.

    Returns:
        overlap1(Array[4,4]): All overlap combinations of the XPM_SPM overlap integral
        overlap2(Array[4]): All overlap combinations of the FWM overlap integral
        zeroing(int): counts how many times the integral was zero.
    """

    #############################################################
    #Creates a list of functions which point to what mode
    #ie what hermit-gaussian polyonomial is used for the overlaps
    B_func = [0, 0, 0 ,0]
...
def field0(y,x,w_vec):
    """
    This function is used to calculate the first order hermit gaussian polyonomial.
    Inputs::
        y (float):: y cartesian coordinate
        x (float):: x cartesian coordinate
        w_vec[array] : The width of the hermit polyonomial ie the width of the mode.
    
    Returns::
        The first hermit-gaussian polyonomial
    """
    w = w_vec[0]
...
def field1(y,x,w_vec):
    """
    This function is used to calculate the second order hermit gaussian polyonomial.
    Inputs::
        y (float):: y cartesian coordinate
        x (float):: x cartesian coordinate
        w_vec[array] : The width of the hermit polyonomial ie the width of the mode.
...
def over1(i,j,B,B_func,w_vec):
    ...
    Inputs::
        i,j (int,int): Integer on what whaves the overlap is calculated for
    ...
    Local::
        ...
        int1,int2,int3,top bottom (float vectors): Integrals (look at Agrawal for the integrals themselves)
    Returns::
        The overlap of the i,j waves.
    """
    if i == j:
        if  B[i] == 'lp01':
            return 1/161
            ...
            return 1/170
    r = 62.45 #The radius of the fibre
    fieldi = B_func[i]
    fieldj = B_func[j]
...
def main(lamp2_start,lamp2_end,lams_start,lams_end,num_points,P1,P2,P_signal,FWM_pros):
    ...
    mat_lp = loadmat('../loading_data/coeffs.mat') #load the coefficients of the inverse group velocity plots
    ...
    dic = {}
    dic['A'] = A
    savemat('Export_data/'+FWM_pros+'.mat',dic)
    return 
    
main(1550,1554,1549,1553,256,30.5,27.,10.5,'phase_conj')
main(1553,1554,1549,1553,256,30.5,27.,10.5,'brag')
import emailing
