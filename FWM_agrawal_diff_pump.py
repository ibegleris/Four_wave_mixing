...
def integrator(int_method,AB0,zmin,zmax,dz,AB_final,i,j,omega,Dk,n2,overlap1,overlap2):
...
def inv_group_disp(i,x,B,mat_lp,lamda_c):
    ...
    if B[i] == 'lp01':
      ...
      coeff = mat_lp['Lp11']
      DD = coeff[0][1]
    return coeff[0][0] +coeff[0][1]*x+coeff[0][2]*x**2, -lamda_c**2*DD/(2*pi*c)
...
def Dk_func(omega,lamda_c,B,zeroing,mat_lp):
  if zeroing == 4:
    return 0
      ...
      b = np.zeros(4)
      for i in range(4):
        inv_group, dispers = inv_group_disp(i,omega[i],B,mat_lp,lamda_c)
...
def FWM(n2,P_vec,P_signal_vec,lamp,lams,lami,dz,num_steps,lamda_c,mat_lp,B,zmin,zmax,zeroing,overlap1,overlap2):
    ...
    for i,Po in enumerate(P_vec):
        ...
        for j,lams_ in enumerate(lams):
            ...
            AB_final[:,i,j],outcome = \
                            integrator(int_method,AB0,zmin,zmax,dz,AB_final,i,j,omega,Dk,n2,overlap1,overlap2)
            if outcome==False:
                ...
                while exits<=55:
                    AB_final[:,i,j],outcome = \
                                   integrator(int_method,AB0,zmin,zmax,dz,AB_final,i,j,omega,Dk,n2)
                    exits +=1
                    dz *= 0.5
...
def main():
  ...
  AB_final = FWM(n2,P_vec,P_signal_vec,lamp,lams,lami,dz,num_steps,lamda_c,mat_lp,B,zmin,zmax,zeroing,overlap1,overlap2)
