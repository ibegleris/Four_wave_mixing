...
def over1(i,j,B,B_func):
    ...
    fieldi = B_func[i]
    fieldj = B_func[j]
    int1 = lambda y,x : np.abs(fieldi(y,x))**2 * np.abs(fieldj(y,x))**2
    top = dblquad(int1,-r,r,lambda x : -r,lambda x: r)[0]

    int2 = lambda y,x : np.abs(fieldi(y,x))**2
    int3 = lambda y,x : np.abs(fieldj(y,x))**2
    bottom = dblquad(int2,-r,r,lambda x : -r,lambda x: r)[0]*\
            dblquad(int3,-r,r,lambda x : -r,lambda x: r)[0]
    return top/bottom


def over2(i,j,k,l,B,B_func):
    ...
    int1 = lambda y,x : fieldi(y,x) * fieldj(y,x) *fieldk(y,x) * fieldl(y,x)
    top = dblquad(int1,-r,r,lambda x : -r,lambda x: r)[0]
    bottom = 1
    for bot in B_func:
        int2 = lambda y,x : np.abs(bot(y,x))**2
        bottom *= dblquad(int2,-r,r,lambda x : -r,lambda x: r)[0]
    bottom **=0.5
    return top/bottom
...
for i in range(4):
    for j in range(4):
        overlap1[i,j] = over1(i,j,B,B_func)
i_vec = np.array([[1,2,3],[0,2,3],[3,0,1],[2,0,1]])
...
for i in range(4):
    ii = i_vec[i,:]
    overlap2[i] = over2(i,ii[0],ii[1],ii[2],B,B_func)
    if overlap2[i] == 0:
        zeroing +=1
...
overlap1 *=1e12
overlap2 *=1e12
zero = np.where(overlap2==0)
print('calculated the overlaps going for the ode')
zero = np.asanyarray(zero)
...
AB_measured = np.zeros([4,len(P_vec),len(lams)])
AB_measured[0,0,:] = D_p
AB_measured[1,0,:] = D_p
AB_measured[2,0,:] = D_s
AB_measured[3,0,:] = D_i
